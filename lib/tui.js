...
function formatTokenCount(value) {
  ...
  const scaledValue = roundedValue / divisor;
  const fixed = `${scaledValue.toFixed(1)}${unit}`;
  if (fixed.length <= COMPACT_TOKEN_WIDTH) return fixed;

  const rounded = `${Math.round(scaledValue)}${unit}`;
  if (rounded.length <= COMPACT_TOKEN_WIDTH) return rounded;

  const scientific = (precision) => `${scaledValue.toExponential(precision).replace('e+', 'e')}${unit}`;
  const preciseScientific = scientific(1);
  return preciseScientific.length <= COMPACT_TOKEN_WIDTH ? preciseScientific : scientific(0);
}
...
function renderDetailDashboard(taskStates, selectedIndex, now, rows = 24) {
  ...
  const usageContentLines = formatTokenUsageLines(selectedTask.lastExecution && selectedTask.lastExecution.usage);
  const fullUsageLines = sectionBox('Last Token Usage', usageContentLines);
  let usageLines = [];
  if (rows >= 18) {
    usageLines = fullUsageLines;
  } else if (rows === 17) {
    usageLines = sectionBox('Last Token Usage', usageContentLines.slice(0, 1));
  }
  const fixedLineCount = headerLines.length + 1 + commandLines.length + fullUsageLines.length + 2;
...
function startTui(runner, options = {}) {
  ...
  function redraw() {
    ...
    const schedule = runner.getSchedule ? runner.getSchedule() : { baseDelayMs: 300_000, jitterMs: 20_000 };
    const frame =
