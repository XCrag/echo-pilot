...
const CLAUDE_WRAPPER_PATH = path.join(__dirname, "..", "bin", "claude-json.js");
...
function createTaskController(commandSpec, dependencies = {}) {
  ...
  const maxStructuredOutputBytes =
    dependencies.maxStructuredOutputBytes ??
  ...
  function attachOutputCapture(runningChild, structuredCapture, generation) {
  ...
  function runOnce({ continueLoop = true, continuousLoop = false } = {}) {
    ...
    attachOutputCapture(runningChild, structuredCapture, generation);
    ...
    runningChild.once("close", (code, signal) => {
      ...
      const lastExecution = normalizeLastExecution({
        provider: commandSpec.name,
        stdout: structuredCapture.enabled
